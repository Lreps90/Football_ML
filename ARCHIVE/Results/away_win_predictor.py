...
data['away_win'] = data['home_goals'] < data['away_goals']
...
data_ready = data.drop(columns=['home_team', 'away_team', 'home_goals', 'away_goals', 'home_odds', 'draw_odds', 'away_odds'])
for league in leagues:
    league_counter += 1
    # Directory where league results are saved
    output_folder = f"away_Leagues_all/{league[0]}_{league[1]}"

    # Check if the directory exists and has 5 files
    if os.path.exists(output_folder) and len(os.listdir(output_folder)) >= 6:
    ...
    for i in range(0, runs):
        ...
        train_data = train_data.drop(columns=['Date', 'away_win', 'League', 'Country'])
        test_data = data_filtered[data_filtered['Date'] > cut_off_date]
        y_test = test_data["away_win"]
        test_data = test_data.drop(columns=['Date', 'away_win', 'League', 'Country'])
                    ...
                    for threshold in thresholds:
                        ...
                        for index, pred in enumerate(y_pred):
                            if pred == 1:
                                ...
                                odds = data.iloc[test_data.index[index]]['home_odds']
